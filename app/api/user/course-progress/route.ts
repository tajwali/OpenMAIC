...
export async function POST(request: Request) {
  try {
    ...
    if (isCompleted) {
      await admin.from('course_progress')
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!body.classroom_id) return NextResponse.json({ error: 'Missing classroom_id' }, { status: 400 })

    const admin = getSupabaseAdmin()
    
    // Fetch current progress to update scenes_completed array
    const { data: current } = await admin.from('course_progress')
...
async function evaluateCompletion(userId: string, classroomId: string, admin: any) {
  const { data: classroom } = await admin.from('classrooms').select('scenes').eq('id', classroomId).single();
