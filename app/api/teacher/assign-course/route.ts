...
export async function GET() {
  try {
    ...
    const classroomMap = new Map((classrooms ?? []).map(c => [c.id, c.title] as [string, string]))
    const studentMap = new Map((studentProfiles ?? []).map(s => [s.id, s.display_name] as [string, string]))
