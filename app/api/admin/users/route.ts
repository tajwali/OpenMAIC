...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const hasChange = body.new_role !== undefined || body.display_name !== undefined ||
      body.new_password !== undefined || body.disabled !== undefined
    ...
    if (body.user_id === auth.user.id && (body.new_role !== undefined || body.disabled)) {
      return NextResponse.json({ error: 'Cannot change your own role or disable your own account' }, { status: 400 })
    }
    ...
    if (body.new_role !== undefined) {
      ...
      const { error } = await admin.from('user_profiles').update({ role: body.new_role }).eq('id', body.user_id)
    ...
    if (body.display_name !== undefined) {
      ...
      const { error } = await admin.from('user_profiles').update({ display_name: name }).eq('id', body.user_id)
      if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    }
    ...
    if (body.disabled !== undefined) {
      ...
      if (!res.ok) {
        const errBody = await res.json().catch(() => ({})) as { msg?: string }
        return NextResponse.json({ error: errBody.msg ?? 'Failed to update user status' }, { status: 500 })
      }
    }
