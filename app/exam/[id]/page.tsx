...
export default function ExamPage() {
  ...
  const submitExam = useCallback(async (forceAnswers?: Record<string, string | string[]>) => {
    if (!exam) return
    setPhase('submitting')
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen text-gray-500">Loading exam…</div>
  if (error) return (
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-md p-8 max-w-md w-full">
          <button onClick={() => router.back()} className="flex items-center gap-1 text-gray-400 hover:text-gray-600 mb-6 text-sm">
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="bg-white border-b sticky top-0 z-10 px-4 py-3 flex items-center justify-between">
          ...
          <span className={`flex items-center gap-1.5 font-mono font-semibold text-sm ${timerWarning ? 'text-red-600' : 'text-gray-700'}`}>
        ...
        <div className="bg-white border-b px-4 py-2 flex gap-1 overflow-x-auto">
          {exam.questions.map((_, i) => {
            const isAnswered = exam.questions[i].type === 'short_answer'
              ? typeof answers[exam.questions[i].id] === 'string' && (answers[exam.questions[i].id] as string).trim().length > 0
              : exam.questions[i].type === 'multiple'
                ? Array.isArray(answers[exam.questions[i].id]) && (answers[exam.questions[i].id] as string[]).length > 0
                : (answers[exam.questions[i].id] as string | undefined) !== undefined && answers[exam.questions[i].id] !== ''
            return (
              <button
                key={i}
              ...
              <div className="space-y-2">
                {(q.options ?? []).map(opt => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-medium mr-2">{opt.value}.</span>{opt.label}
                    </button>
                  )
                })}
              </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => router.push('/')} className="flex items-center gap-1 text-gray-400 hover:text-gray-600 mb-6 text-sm mt-4">
            <ArrowLeft className="w-4 h-4" /> Dashboard
          </button>

          <div className={`rounded-2xl p-6 mb-6 text-center ${bg}`}>
            <p className={`text-6xl font-bold ${color} mb-2`}>{Math.round(pct)}%</p>
            <p className="text-gray-600 text-sm">{result.score} / {result.total} points</p>
                ...
                <div key={a.questionId} className="bg-white rounded-2xl shadow-sm p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <p className="text-sm font-medium text-gray-800 flex-1">
                      <span className="text-gray-400 mr-1">Q{i + 1}.</span>{a.question}
                    </p>
                    {isShort ? (
                  ...
                  {a.submitted !== undefined && a.submitted !== '' && !(Array.isArray(a.submitted) && a.submitted.length === 0) && (
                  ...
                  {a.analysis && (
                    <p className="text-xs text-gray-400 mt-2 italic">{a.analysis}</p>
                  )}

                  <p className="text-xs text-gray-400 mt-2">
                    {isShort ? `${a.points} pt${a.points !== 1 ? 's' : ''} (not auto-graded)` : `${a.earned} / ${a.points} pt${a.points !== 1 ? 's' : ''}`}
                  </p>
                </div>
              )
            })}
